    ...
    >
      <div className="container max-w-7xl mx-auto px-4 md:px-8  py-12 md:py-16 lg:py-20">
        ...
        <div className="text-center mb-8 md:mb-12">
          ...
          <Link to="/add-coffee">
            <button className="inline-flex items-center gap-2 primary-text text-white text-xl md:text-2xl font-semibold px-4 md:px-6 py-2 rounded border-2 border-[#331A15] bg-[#E3B577] hover:bg-transparent hover:text-[#331A15] transition-colors tracking-wide cursor-pointer">
              Add Coffee
              <BsFillCupHotFill
                className="drop-shadow-[2px_2px_4px_#1b1a1a]"
              />
            </button>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 max-w-6xl mx-auto">
          {coffees.map((coffee) => (
            <CoffeeCard key={coffee._id} coffee={coffee} onDelete={onDelete} />
          ))}
        </div>
      </div>
      <img
